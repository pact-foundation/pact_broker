  ...
  module Logging
    ...
    module PayloadSanitizer
      ...
      def self.call(value, depth = 0)
        ...
        when Hash then sanitize_hash(value, depth)
        when Array then sanitize_array(value, depth)
      ...
      def self.sanitize_hash(hash, depth)
        return DEPTH_EXCEEDED if depth >= MAX_DEPTH

        hash.each_with_object({}) do |(key, value), new_hash|
          new_hash[call(key, depth + 1)] = call(value, depth + 1)
        end
      end
      private_class_method :sanitize_hash

      def self.sanitize_array(array, depth)
        return DEPTH_EXCEEDED if depth >= MAX_DEPTH

        array.collect { |value| call(value, depth + 1) }
      end
