...
task :'pact_broker:routes', [:search_term] do | _t, args |
  ...
  Tempfile.create("pact_broker_routes") do |f|
    ...
    routes.each do | route |
      ...
      if route[:schemas]
        puts "      schemas:"
        route[:schemas].each do | http_method, schema_details |
          puts "            method: #{http_method}"
          puts "            class: #{schema_details[:class]}"
          puts "                   location: #{schema_details[:location]}"
...
def methods(route)
  route.allowed_methods.each_with_object({}) do | meth, methods |
    method_info = {}
    methods[meth.downcase] = method_info

    if route.schemas && route.schemas[meth]
      if meth == "GET"
        method_info["parameters"] = route.json_schema_for(meth)
      elsif meth == "POST"
        method_info["requestBody"] = route.json_schema_for(meth)
      end
    end
  end
end
...
desc "Generate an OAS document"
task :'pact_broker:oas', [:search_term] do | _t, args |
  project_root = File.absolute_path(File.join(__dir__, "..", "..", ".."))
  $LOAD_PATH << File.join(project_root,"app_shared","lib")
  ...
  Tempfile.create("pact_broker_routes") do |f|
    ...
    paths = routes.each_with_object({}) do | route, p |
      p[route.path] = methods(route)
    end

    require "yaml"

    puts ({ "paths" => paths }).to_yaml
