    ...
    describe "override_runtime_configuration!" do
      ...
      context "when the specified runtime attribute does not exist" do
        it "logs that it has ignored those attributes" do
          allow(config).to receive(:logger).and_return(logger)
          expect(logger).to receive(:debug).with("Overriding runtime configuration", hash_including(ignoring: { no_exist: true }))
          config.override_runtime_configuration!(no_exist: "true")
        end
      end
