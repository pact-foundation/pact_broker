    ...
    describe VerifiablePactMessages do
      ...
      describe "#inclusion_reason" do
        context "when there are no head consumer tags" do
          its(:inclusion_reason) { is_expected.to eq "Verifying the latest pact between Foo and Bar." }
        end

        context "when there is 1 head consumer tags" do
          let(:head_consumer_tags) { %w[dev] }
          its(:inclusion_reason) { is_expected.to eq "Verifying the pact for the latest version of Foo tagged with 'dev'" }
        end

        context "when there are 2 head consumer tags" do
          let(:head_consumer_tags) { %w[dev prod] }
          its(:inclusion_reason) { is_expected.to eq "Verifying the pact for the latest versions of Foo tagged with 'dev' and 'prod' (both have the same content)" }
        end

        context "when there are 3 head consumer tags" do
          let(:head_consumer_tags) { %w[dev prod feat-x] }
          its(:inclusion_reason) { is_expected.to eq "Verifying the pact for the latest versions of Foo tagged with 'dev', 'prod' and 'feat-x' (all have the same content)" }
        end
      ...
      describe "#pending_reason" do
        context "when the pact is not pending" do
          context "when there are no non_pending_provider_tags" do
            its(:pending_reason) { is_expected.to eq "This pact has previously been successfully verified by Bar. If this verification fails, it will fail the build." }
          end

          context "when there is 1 non_pending_provider_tag" do
            let(:non_pending_provider_tags) { %w[dev] }

            its(:pending_reason) { is_expected.to eq "This pact has previously been successfully verified by a version of Bar with tag 'dev'. If this verification fails, it will fail the build."}
          end
        end
        ...
        context "when the pact is pending" do
          ...
          context "when there are no pending_provider_tags" do
            context "when there are no non_pending_provider_tags" do
              its(:pending_reason) { is_expected.to eq "This pact is in pending state because it has not yet been successfully verified by Bar. If this verification fails, it will not cause the overall build to fail." }
            end
          end
          ...
          context "when there is 1 pending_provider_tag" do
            ...
            its(:pending_reason) { is_expected.to eq "This pact is in pending state because it has not yet been successfully verified by any version of Bar with tag 'dev'. If this verification fails, it will not cause the overall build to fail." }
          end
