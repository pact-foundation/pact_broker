...
REQUESTS_WHICH_ARE_EXPECTED_TO_HAVE_NO_POLICY_RECORD = YAML.safe_load(File.read("spec/support/all_routes_spec_support.yml"))["requests_which_are_expected_to_have_no_policy_record_or_pacticipant"]
...
# This test is here to ensure that every new route added to the Pact Broker API has been considered, and explicitly whitelisted if necessary.
PactBroker.routes.each do | pact_broker_route |
  describe "#{pact_broker_route.path} (#{pact_broker_route.resource_name})" do
    (pact_broker_route.allowed_methods - ["OPTIONS"]).each do | allowed_method |
      ...
      if !REQUESTS_WHICH_ARE_EXPECTED_TO_HAVE_NO_POLICY_RECORD.include?("#{pact_broker_route.resource_name} #{allowed_method}")
